...
\begin{definition} \label{def:probablity-value}
  A $p\in \Real$ is a probability value when $p \ge 0 \wedge p \le 1$.
  \lean{Probability.Prob}
\end{definition}

The lean file also defines several basic properties for \cref{def:probablity-value} which we do not summarize here.

We also need some properties that help us to manipulate lists. 
\begin{definition}
A list $L \colon \mathcal{N} \to \tau$ of type $\tau$ and length $N$ is a function defined for $\mathcal{N} := \left\{ 0, \dots , n-1 \right\}$ of type $\tau$.
\lean{List} 
\end{definition}

\begin{definition} \label{def:lsimplex}
A list $L\colon \mathcal{N} \to \tau$ and length $N$
\end{definition}
