...
\lean{Probability.Probability.Prob}
\end{definition}
...
It is easy to show that a probability space cannot be emtpy.
\begin{theorem}
...
We use the operator (function) $\mathbb{P}$ denote the probability of the set of sample space that corresponds to a given condition. The operator is defined as follows.
\begin{definition}
  For any probability space, $(\Omega, \mathcal{F}, P)$ and a random variable $\tilde{x}\colon \Omega \to \Real$. Assume that $\Real$ is endowed with some $\sigma$-algebra $\mathcal{E}\subseteq 2^{\Real}$. Then, the operator $\PP$ is defined as:
\[
\PP\left[\tilde{x} = x_0\right] := P(x^{-1}(x_0)), \qquad \forall x_0\in \Real, 
\]
and, more generally,
 \[
   \PP\left[\tilde{x} \in \mathcal{X}\right] := P(x^{-1}(\mathcal{X})), \qquad \forall \mathcal{X} \in \mathcal{E}. 
 \]
 Here, the pre-image applied to a set is defined as in \eqref{eq:preimage-set}.
\end{definition}


Consider a random variable $\tilde{b}\colon \Omega \to \left\{ 0,1 \right\}$ defined on the probability space from \cref{exm:probability-space}. 
\[
  \P\left[\tilde{b} = 1\right] := P\left(b^{-1}(1)\right) .
\]
As before, the set-valued function $b^{-1}\colon \left\{ 0,1 \right\} \to \mathcal{F}$ is the \emph{pre-image} of $b$ as in \cref{qn:tails-definition}.



We use the operator (function) $\mathbb{P}$ denote the probability of the set of sample space that corresponds to a given condition. The operator is defined as follows.
\begin{definition}
  For any probability space, $(\Omega, \mathcal{F}, P)$ and a random variable $\tilde{x}\colon \Omega \to \Real$. Assume that $\Real$ is endowed with some $\sigma$-algebra $\mathcal{E}\subseteq 2^{\Real}$. Then, the operator $\P$ is defined as:
  \[
   \P\left[\tilde{x} = x_0\right] := P(x^{-1}(x_0)), \qquad \forall x_0\in \Real, 
 \]
 and, more generally,
 \[
   \P\left[\tilde{x} \in \mathcal{X}\right] := P(x^{-1}(\mathcal{X})), \qquad \forall \mathcal{X} \in \mathcal{E}. 
 \]
 Here, the pre-image applied to a set is defined as in \eqref{eq:preimage-set}.
\end{definition}
...
\begin{definition}[CDF]
  ...
  \[
   F_{\tilde{x}}(x) := \P\left[\tilde{x} \le x\right], \qquad \forall x\in \Real. 
  \]
\end{definition}

The CDF is always non-decreasing and right-continuous.
...
The \emph{probability mass function} $p_{\tilde{x}}\colon \mathcal{E} \to [0,1]$ for a discrete random variable $\tilde{x}\colon \Omega  \to  \mathcal{E}$ with a finite $\mathcal{E}$ is defined as
  \[
    p_{\tilde{x}}(e) := \P\left[\tilde{x} = e\right], \qquad \forall e\in \mathcal{E} .
 \]
\end{definition}
...
\[
 \P\left[\tilde{x} = x, \tilde{y} = y\right] := \P\left[\tilde{x} = x \wedge \tilde{y} = y\right] , \forall  x\in \mathcal{X}, y\in \mathcal{Y}.
\]
...
\[
 p_{\tilde{x}, \tilde{y}}(x,y) = \P\left[\tilde{x} = x, \tilde{y} = y\right]. 
\]
\end{definition}
...
\[
  \P\left[\tilde{x} = x, \tilde{y} = y\right] = \P\left[\tilde{x} = x\right] \cdot \P\left[\tilde{y} = y\right], \forall x\in \mathcal{X}, y\in \mathcal{Y}.
\]


The expectation of a random variable is its mean.
...
\[ 
 \E\left[\tilde{x}\right] := \sum_{x\in \tilde{x}(\Omega)}  x\cdot \P\left[\tilde{x} = x\right].
\]
\end{definition}
In general, expectation is defined using the Lebesgue integral:
\[
 \E\left[\tilde{x}\right] := \int_{\Omega} \tilde{x} \, dP,
\]
but that is beyond the formalism that we cover in this class. 
...
\begin{theorem}[Law of the unconscious statistician] \label{thm:unconscious}
  ...
  \[
    \E\left[g(\tilde{x})\right] = \sum_{x\in \mathcal{X}} g(x) \cdot  \P\left[\tilde{x} = x\right].
  \]
\end{theorem}
...
\begin{theorem}
  ...
  \begin{align*}
    \E\left[x\cdot \tilde{x}\right] &= x \cdot  \E\left[\tilde{x}\right]\\
